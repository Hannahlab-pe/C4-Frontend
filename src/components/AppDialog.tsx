...
export default function AppDialog({ open, onClose, title, wide, children }: AppDialogProps) {
  return (
    <Dialog open={open} onClose={onClose} className="relative z-50">
      ...
      <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
        <DialogPanel
          transition
          className={`w-full ${wide ? 'max-w-2xl' : 'max-w-lg'} bg-white rounded-2xl shadow-xl max-h-[90vh] flex flex-col transition duration-200 ease-out data-closed:scale-95 data-closed:opacity-0`}
        >
